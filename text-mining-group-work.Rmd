...
corpus <- tm_map(corpus, content_transformer(function(x) {
  ...
  x <- gsub("…", "", x) 
  x <- gsub("«", "", x) 
  x <- gsub("rt", "", x, ignore.case = TRUE)  # Remove 'rt' used in retweets
...
```{r, fig.width=15, fig.height=10}
# Code um Tweet-Frequenzen nach Zeit und Universität zu analysieren
# Fokus nur auf BFH
bfh_tweets <- filtered_tweets %>%
  filter(university == "bfh") %>%  # Filter um nur BFH zu berücksichtigen
  mutate(tweet_month = floor_date(created_at, "month")) %>%
  group_by(tweet_month) %>%
  summarize(count = n(), .groups = 'drop')

# Verwendung von scale_fill_gradient() für einen Farbverlauf
ggplot(bfh_tweets, aes(x = tweet_month, y = count, fill = count)) +
  geom_col() +
  scale_fill_gradient(low = "lightblue", high = "blue") +
  theme_minimal() +
  labs(title = "Monthly Tweet Frequency for BFH", x = "Year", y = "Number of Tweets")
```

```{r, fig.width=15, fig.height=10}
# Angenommen, es gibt spezielle Ereignisse im Mai jedes Jahres
bfh_tweets$event <- ifelse(format(as.Date(bfh_tweets$tweet_month), "%m") == "05", "Event", "Normal")

# Visualisierung mit spezieller Farbgebung für Ereignisse
ggplot(bfh_tweets, aes(x = tweet_month, y = count, fill = event)) +
  geom_col(show.legend = TRUE) +
  scale_fill_manual(values = c("Event" = "red", "Normal" = "gray")) +
  theme_minimal() +
  labs(title = "Monthly Tweet Frequency for BFH", x = "Year", y = "Number of Tweets")
