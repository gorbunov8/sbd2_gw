...
corpus <- Corpus(VectorSource(filtered_tweets$full_text))
```
...
```{r}
# Beispiel: Erstellung eines Balkendiagramms der häufigsten Wörter
word_data <- data.frame(Word = names(word_freq), Frequency = unname(word_freq))
ggplot(word_data[1:10, ], aes(x = reorder(Word, -Frequency), y = Frequency)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Top 10 häufigste Wörter", x = "Wörter", y = "Häufigkeit")
```
